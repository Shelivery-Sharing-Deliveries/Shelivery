        ...
        const fetchChatroomsData = async () => {
            ...
            try {
                ...
                if (userError || !userData?.dormitory?.[0]?.name) {
                    ...
                    throw new Error("Could not retrieve user's dormitory information. Please ensure your profile is complete.");
                }
                const dormitoryName = userData.dormitory[0].name;
                ...
                const uniqueChatroomIds = Array.from(new Set(userBaskets.map((b) => b.chatroom_id).filter(Boolean))) as string[];
                const uniqueShopIds = Array.from(new Set(userBaskets.map((b) => b.shop_id).filter(Boolean))) as string[];
                ...
                const chatroomsWithCombinedData = await Promise.all(
                    fetchedChatrooms.map(async (chatroom) => {
                        ...
                        const relatedBasket = userBaskets.find((b) => b.chatroom_id === chatroom.id);
                        ...
                        if (messageError && messageError.code !== 'PGRST116') { // PGRST116 means no rows found (no messages)
                            console.error(`Error fetching latest message for chatroom ${chatroom.id}:`, messageError);
                        }
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {chatroom.last_message_at && (
                                            <p className="text-sm text-shelivery-text-secondary mt-1">
                                                Last activity: {new Date(chatroom.last_message_at).toLocaleString()}
                                            </p>
                                        )}
                                        {!chatroom.last_message_at && (
