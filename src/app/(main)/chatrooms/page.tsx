    ...
    useEffect(() => {
        const fetchChatroomsData = async () => {
            ...
            try {
                ...
                const uniqueChatroomIds = [...new Set(userBaskets.map((b) => b.chatroom_id).filter(Boolean))] as string[];
                const uniqueShopIds = [...new Set(userBaskets.map((b) => b.shop_id).filter(Boolean))] as string[];
