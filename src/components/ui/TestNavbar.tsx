...
const navItems = [
  ...
  },
  {
    name: "Signup",
    href: "/auth",
    icon: "user", // I'll need to find an appropriate icon for signup
  },
];
...
export default function TestNavbar() {
  ...
  const renderIcon = (iconType: string, isActive: boolean) => {
    ...
    } else if (iconType === "user") { // I'll add a case for the user icon
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-20 w-full bg-[#245B7B]">
      <div
        className={`w-full h-[74px] flex justify-center items-center gap-[45px] px-4 py-1.5 pb-6`}
      >
        {navItems.map((item) => {
          const isActive =
            pathname === item.href ||
            pathname.startsWith(item.href + "/") ||
            (item.href === "/test/choose-shop" && pathname.startsWith("/test/create-basket"));
